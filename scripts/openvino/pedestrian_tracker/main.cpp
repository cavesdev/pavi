...
int main_work(int argc, char **argv) {
    ...
    bool should_save_video = !FLAGS_no_save;
    int frame_skip = FLAGS_skip;
    bool should_save_json = FLAGS_json;
    ...
    if (should_save_video) {
        std::string::size_type dot = FLAGS_i.find_last_of('.');
        std::string output_filename = FLAGS_i.substr(0, dot) + ".avi";
        cv::Size output_dimensions = cv::Size((int) cap.get(cv::CAP_PROP_FRAME_WIDTH),
                            (int) cap.get(cv::CAP_PROP_FRAME_HEIGHT));
        output_video.open(output_filename, cv::VideoWriter::fourcc('M','J','P','G'), video_fps, output_dimensions);

        if (!output_video.isOpened())
        {
            std::cout << "Could not open the output video for write: " << FLAGS_i << std::endl;
            return -1;
        }
    }
    
    /* save JSON with results */
    JSONWriter *output_json = new JSONWriter(&cap, FLAGS_i);
    ...
    for (int32_t frame_idx = std::max(0, FLAGS_first); 0 > FLAGS_last || frame_idx <= FLAGS_last; ++frame_idx) {
        ...
        //add results to json
        if (should_save_json) {
            output_json->add_to_json(detections, frame_idx, cur_timestamp);
        }
