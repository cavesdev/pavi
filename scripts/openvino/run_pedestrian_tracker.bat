...
set VIDEO_DIR=%PROJECT_DIR%\%UPLOAD_FOLDER%\videos
...
set target_video_path=%VIDEO_DIR%\%video%
...
for /F "tokens=1,2 usebackq" %%a in ("%ROOT_DIR%run_pedestrian_tracker.conf") do (
...
echo.
timeout 3
echo "!MSBUILD_BIN!" Demos.sln /p:Configuration=Release /t:pedestrian_tracker /clp:ErrorsOnly /m
"!MSBUILD_BIN!" Demos.sln /p:Configuration=Release /t:pedestrian_tracker /clp:ErrorsOnly /m
if ERRORLEVEL 1 GOTO errorHandling
...
:runSample
echo.
echo ###############^|^| Run Inference Engine Pedestrian Tracker ^|^|###############
echo.
timeout 3
cd "%SOLUTION_DIR64%\intel64\Release"
echo "%SOLUTION_DIR64%\intel64\Release\pedestrian_tracker.exe" -i "%target_video_path%"%model_args% -d !TARGET! -d_va !TARGET! -d_lpr !TARGET! !SAMPLE_OPTIONS!
echo %model_args%
pedestrian_tracker.exe %model_args% -i "%target_video_path%" -d !TARGET! !SAMPLE_OPTIONS!
if ERRORLEVEL 1 GOTO errorHandling
